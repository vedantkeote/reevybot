...
export default {
    ...
    callback: async({ message, interaction, args, client }) => {
        ...
        if (targetMessage.author.id !== client.user?.id) {
            return `Please provide a message ID that was sent by the user <@${client.user?.id}> ID: ${client.user?.id}`
        }
        ...
        if (menu) {
            ...
            row.addComponents(
                new MessageSelectMenu() 
                    ...
                    .setMinValues(0)
                    .setMaxValues(1)
                    .setPlaceholder('Choose roles to add from the menu below...')
                    .addOptions(option)
            )
        }
        ...
        return {
            custom: true,
            content: `Added <@&${role.id}> to the role selection menu.`,
            allowedMentions: {
                roles: [],
            },
            ephemeral: true,
